...
let journeys = [];
journeys = loadJSON("C:/Users/Charles/Documents/Polytechnique/Hiver2018/CIV6707A/PROJET/CodeAlgo/journeys.json");
...
for (let i = 0, count = stops.length; i < count; i++){
  ...
  //INITIALISER LES ARRÊTS DE DÉPART POTENTIELS!!!
...
const footpathsTransfertTest = footpathsTransfertById.get(443);
...
for (let i = 0, count < connections.length; i < count; i++){
  ...
  if (tripsMap.get(connections[i].trip_id).connection || stopsMap.get(connections[i].stop_id_DEPART) <= connections[i].departure_time){
    if (!tripsMap.get(connections[i].trip_id).connection){
    ...
    for (let j = 0, count < transferts.length; j < count; j++){ //transferts = footpathsTransfertById.get(connections[i].stop_id_ARRIVEE);
      if (connections[i].heure_arrivee + transferts[j].travelTime < stopsMap.get(transferts[j].toStopId).arrival_time){
        stopsMap.get(transferts[j].toStopId).arrival_time = connections[i].heure_arrivee;
